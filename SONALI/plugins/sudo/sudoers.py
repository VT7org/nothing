...
async def sudoers_list(client, message: Message):
    ...
    await message.reply_text(
        "**» ᴄʜᴇᴄᴋ sᴜᴅᴏ ʟɪsᴛ ʙʏ ɢɪᴠᴇɴ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ.**\n\n**» ɴᴏᴛᴇ:**  ᴏɴʟʏ sᴜᴅᴏ ᴜsᴇʀs ᴄᴏᴜʟᴅ ᴠɪᴇᴡ ᴛʜɪs ʟɪsᴛ.",
...
async def check_sudo_list(client, callback_query: CallbackQuery):
    if callback_query.from_user.id not in SUDOERS:
        return await callback_query.answer("  sᴏʀʀʏ!  \n ᴏɴʟʏ ᴍʏ ᴏᴡɴᴇʀ ᴄᴏᴜʟᴅ sʜᴏᴡ ʏᴏᴜ ᴛʜᴀᴛ  ", show_alert=True)

    keyboard = []
    user = await app.get_users(OWNER_ID)
    user_mention = user.mention if user else "Owner"

    caption = f"**˹ʟɪsᴛ ᴏғ ʙᴏᴛ ᴍᴀɴᴀɢᴇʀs **\n\n**Oᴡɴᴇʀ** ➥ {user_mention}\n\n"
    ...
    for user_id in SUDOERS:
        if user_id != OWNER_ID:
            try:
                ...
                button_text = f"ᴠɪᴇᴡ {count} sᴜᴅᴏ ᴜsᴇʀ"
...
async def back_to_main_menu(client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_caption(
        caption="**» ᴄʜᴇᴄᴋ sᴜᴅᴏ ʟɪsᴛ ʙʏ ɢɪᴠᴇɴ ʙᴜᴛᴛᴏɴ.**\n\n**» ɴᴏᴛᴇ:**  ᴏɴʟʏ sᴜᴅᴏ ᴜsᴇʀs ᴄᴏᴜʟᴅ ᴠɪᴇᴡ ᴛʜɪs.",
        reply_markup=reply_markup
    )


@app.on_message(filters.command(["rmallsudo"], prefixes=["/", "!", "%", ",", ".", "@", "#"]) & filters.user(OWNER_ID))
@language
